...
class CrmLead(models.Model):
    ...
    def get_sa_dashboard_values(self):
        ...
        def compute_metrics(filter_context, Label):
            ...
            won_count           = won_data[0]['__count'] if won_data and won_data[0]['__count'] is not None else 0
            closed_amount       = won_data[0]['expected_revenue'] if won_data and won_data[0]['expected_revenue'] is not None else 0.0
            sum_days_to_win     = won_data[0]['day_close'] if won_data and won_data[0]['day_close'] is not None else 0.0
            open_count          = open_data[0]['__count'] if open_data and open_data[0]['__count'] is not None else 0
            lost_count          = lost_data[0]['__count'] if lost_data and lost_data[0]['__count'] is not None else 0
            avg                 = self.env['crm.lead'].read_group(domain, ['expected_revenue:avg'], [])
            avg_deal_size       = avg[0]['expected_revenue'] if avg and avg[0]['expected_revenue'] is not None else 0
            ...
            _logger.error(won_data)
            _logger.error(open_data)
            _logger.error(lost_data)
